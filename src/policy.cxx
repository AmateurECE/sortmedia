...
public:
  PolicyResult apply(PendingCopyFileTransaction& transaction) final {
    ...
    const auto artist{sanitize_token(file.tag()->artist().to8Bit(true))};
    const auto album{sanitize_token(file.tag()->album().to8Bit(true))};

    const auto alignment{
        calculate_track_number_alignment(file, transaction.source_path())};
    const auto track_number{file.tag()->track()};
    const auto title{sanitize_token(file.tag()->title().to8Bit(true))};
    ...
    transaction.set_destination_path(
        fs::path(artist).append(album).append(track));
    return {monostate()};
  }

private:
  static inline constexpr unsigned int MINIMUM_ALIGNMENT = 2;
  unordered_map<fs::path, unsigned int> m_track_total_cache;
  ...
  unsigned int calculate_track_number_alignment(const TagLib::FileRef& file,
                                                const fs::path& source_path) {
    NonStandardTags tags{file};
    auto track_total{tags.track_total().or_else(
        [this, &source_path]() -> optional<unsigned int> {
  ...
  unsigned int track_total_heuristic(const fs::path& file) {
...
public:
  PolicyResult apply(PendingCopyFileTransaction& transaction) final {
    ...
    auto image_detector{ImageDetector::make(transaction.source_path())};
    ...
    for (const auto& filename : artwork_filenames) {
      auto art{fs::path(source_directory).append(filename)};
      if (fs::exists(art)) {
        transaction.add_action([art](fs::path file) {
          auto destination{file.append(art.filename().c_str())};
          cout << art.filename() << "\n";
          fs::copy_file(art, destination);
        });
...
private:
  unordered_set<fs::path> m_collection_cache;

  static vector<string> known_artwork_filenames() {
    static const vector<string> extensions = {".jpg", ".png"};
    static const vector<string> filenames = {"folder", "poster", "cover",
                                             "default"};
